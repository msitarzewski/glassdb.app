...
### Approved Workspace and SQL Editor Polish (2026-09-05)
- Final generic iOS/iPadOS and visionOS simulator builds passed on this shared-source revision before PR publication; these are compile checks, not physical-device UI acceptance.
- Mac titlebar frost uses passive, non-hit-testing `.sidebar` material that reads the native sidebar boundary; never constrain decoration to live split-view columns. Keep `.fullSizeContentView`, hide the competing SwiftUI window-toolbar background, contain tab-strip material within its safe area, and let AppKit choose toolbar sizing. New Mac workspaces open the schema sidebar once after native activation/restoration; later user toggles are respected. Sources: `glassdb/Constants.swift`, `glassdb/DatabaseWorkspaceView.swift`, `glassdb/glassdbApp.swift`, `glassdb/SettingsView.swift`.
- Grid text/headers use proportional system fonts; typed numeric values use monospaced digits and trailing alignment. Textual identifiers remain textual. Resize handles are small capsules with larger hit targets; the viewport has an 8pt trailing inset. Empty SQL tabs mirror the top-aligned table editor and blank striped results canvas without fabricating query results. Source: `glassdb/TableDetailView.swift`.
- Shared Run control: Command-Return executes selection, otherwise the caret statement; Shift-Command-Return executes all. Explicit Statement/Selection/All actions reuse the SQL parser and mutation confirmation. Holding Shift+Command changes the Mac label/action to All Statements. Generated table SQL ends in a semicolon; batches stop on thrown or returned server errors. Sources: `glassdb/QueryEditorView.swift`, `glassdb/SQLHighlighter.swift`, `glassdb/Constants.swift`.
- Qualified completion lazily loads live tables for the database typed before the dot, independently of the selected database, in both editors. Tab accepts completion and advances over a trailing space, reusing existing whitespace and preserving punctuation. No full multi-database column scan is added. Sources: `glassdb/TableDetailView.swift`, `glassdb/SQLHighlighter.swift`.
- Workspace-owned table SQL drafts share the SQL-file exporter and tab-close Save Changes flow (Return Save, Command-D Don't Save, Escape Cancel, Command-S explicit save). Generated SQL is a clean baseline; unsaved edits survive browsing refresh. **Not complete data-loss protection:** the user reported edited SQL was still not captured; the exact action/prompt path is unconfirmed. Whole-window close/app quit and durable draft recovery are not implemented. Do not describe this report as fixed. Source: `glassdb/DatabaseWorkspaceView.swift`.
- Evidence: user approved installed Mac appearance, empty-editor layout, completion, and Tab spacing. Final Mac suite passed 145/145 with zero failures/skips; xcresult includes five negative-width geometry runtime warnings in the test host, so this is not a warning-free run. Signed Release was verified and installed with rollback retained (executable SHA-256 `f23ef5c60fd0edf857b5b82d46a7b3369da46635d04f0d4d534e1bfb28dc6047`). No live database mutations were used for automated validation. Final shared-platform builds and CI evidence are recorded in the PR; physical-device and native save-dialog acceptance remain separate gates.
